...
if not fs.exists(STARTUP_FILE) then
	Util.writeFile(STARTUP_FILE,
		[[os.sleep(1)
shell.openForegroundTab('packages/miloApps/apps/compress')]])
end
		...
		local s, m = pcall(function()
			...
			if cooking.count < 64 and inItem and inItem.count > 0 then
				if cooking.count == 0 or cooking.name == inItem.name then
					...
					local count = inv.pushItems(furnace.dev.name..".top", INPUT_SLOT, SMELT_AMOUNT, INPUT_SLOT)
...
--					local count = furnace.dev.pullItems(localName, FUEL_SLOT, 8 - fuel.count, FUEL_SLOT)
					local count = inv.pushItems(furnace.dev.name..".bottom", FUEL_SLOT, 8 - fuel.count, FUEL_SLOT)
					if count > 0 then
			...
			if result.count > 0 then
				if inReturn.count == 0 or result.name == inReturn.name then
					...
					local count = inv.pullItems(furnace.dev.name..".north", OUTPUT_SLOT, result.count, OUTPUT_SLOT)
